...
test('overlapsAny true when identical range is in compare', () => {
  ...
  const ranges = [closedRange(10, 30), soughtRange, closedRange(4, 10), closedRange(33, 43)];
...
test('overlapsAny true when sought range ends on compare range start', () => {
  ...
  const ranges = [closedRange(100, 101), closedRange(93, 204), barelyOverlaps, closedRange(-100, -90)];
...
test('overlapsAny false when sought range ends on compare range start - 1', () => {
  ...
  const ranges = [closedRange(100, 101), closedRange(93, 204), justOutside, closedRange(-100, -90)];
